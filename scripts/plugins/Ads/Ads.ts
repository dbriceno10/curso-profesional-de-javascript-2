...
export interface Ad {
    imageUrl: string
    title: string
    body: string //Esto lo vamos a guardar en el body
    url: string //Si el usuario da click en el anuncion queremos enviarlo a una url del anunciante
}
...
class Ads {
  ...
  private constructor() {
      this.initAds() //Recordar que esto es que estamos inicializando el método en el constructor
  }
  ...
  private initAds() {
    this.ads = [...ALL_ADS]//Con los 3 puntos suspensivos antes, estamos "dejando caer" el contenido del arreglo original en otro arreglo, es decir, estamos haciendo una copia del arreglo sin modificar el original
  }

  getAd() {
    if (this.ads.length === 0) {
      this.initAds() //Si seacaban los ads, los vamos a volver a inicializar
    }
    return this.ads.pop()
  }
}
